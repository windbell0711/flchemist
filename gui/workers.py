...
from main import reverse_from_log
...
class ExecuteWorker(QtCore.QThread):
    ...
    def run(self):
        ...
        self._write_log({"event": "start", "total": result.total})
        ...
        for i, act in enumerate(self._actions, 1):
            ...
            try:
                ...
                self._write_log({"event": "action_done", "index": i, "desc": act.desc, "status": "done", "action_data": action_to_dict(act)})
                self._log.info("Action %d/%d done: %s", i, result.total, act.desc)
            except Exception as e:
                result.failed += 1
                self._log.error("Action %d/%d failed: %s - %s", i, result.total, act.desc, str(e))
                tb = "".join(traceback.format_exception(type(e), e, e.__traceback__))
                ...
                if decision == "abort":
                    ...
                    try:
                        ...
                        self._write_log({"event": "reverse_single", "index": i, "desc": act.desc, "status": "done"})
                        ...
                        self._write_log({"event": "reverse_single", "index": i, "desc": act.desc, "status": "failed", "error": rev_tb})
                    break
                    ...
                    for rev_idx, rev_act in reversed(completed):
                        try:
                            rev_act.reverse()
                            self._write_log({"event": "reverse_all", "index": rev_idx, "desc": rev_act.desc, "status": "done"})
                        except Exception as rev_e:
                            rev_tb = "".join(traceback.format_exception(type(rev_e), rev_e, rev_e.__traceback__))
                            result.reverse_errors.append(f"[rollback all] #{rev_idx} {rev_act.desc}: {rev_e}")
                            self._write_log({"event": "reverse_all", "index": rev_idx, "desc": rev_act.desc, "status": "failed", "error": rev_tb})
                    break

        self._write_log({"event": "finish", "success": result.success, "failed": result.failed, "decision": result.decision.value if result.decision else None})
...
class ReverseWorker(QtCore.QThread):
    _log = logging.getLogger("flchemist.ReverseWorker")
    ...
    def run(self):
        try:
            success, failed = reverse_from_log(self._log_file)
            self._log.info("Reverse completed: %d success, %d failed", success, len(failed))
